...
for c in df.columns[3:]:
    ...
    df_temp = pd.DataFrame(
        df[c].apply(threshold_uniformity, thresholds=thresholds).astype(str)
    )
    new_columns = (
        df_temp.join(df_temp[c].str.get_dummies()).drop(columns=[c]).add_prefix(c + "_")
    )
...
# save to CSV
pd.DataFrame(exc_results, columns=headers).to_csv("output.csv", index=False, sep=";")
