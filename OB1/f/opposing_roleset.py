...
df = df[df['oppo_deck_colors'].notna()]
...
def conditional_winrate(desc, roleset):
    d_str = " and ".join([as_string(desc), 'oppo_deck_colors=="'+ roleset +'"'])
...
def eval_quality(desc):
    # get role set set
    rss = df[df.eval(as_string(desc)).astype(bool)]["oppo_deck_colors"].unique()
...
pd.DataFrame(exc_results, columns=headers).to_csv(
    "Code/OB1/f/output_opp_rolesets.csv", index=False, sep=";"
)
